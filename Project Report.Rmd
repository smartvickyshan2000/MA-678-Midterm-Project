...
Look into the data and finished the following processing steps: 

1. Subset down to 10 out of 23 variables;
...
After my data cleaning and processing, the dataset utilized later has 7081 observations and 10 variables besides customer type.  

10 Variables Utilized:  
...
pie_existing_gender <- data.frame(class = c("Female", "Male"), Prop = c(53.10, 46.90))
# Add label position
...
pie_attrited_gender <- data.frame(class = c("Female", "Male"), Prop = c(51.75, 48.25))
# Add label position
...
##### set pie 2: Gender
pie2 <- data.frame(class = c("Female", "Male"), Prop = c(47.66, 52.34))
# Add label position
pie2 <- pie2 %>%
  arrange(desc(class)) %>%
  mutate(lab.ypos = cumsum(Prop) - 0.5*Prop)
...
pie3 <- data.frame(class = c("College", "Doctorate", "Graduate", "High School", "Post-Graduate",
                             "Uneducated"), 
                   Prop = c(11.92, 5.06, 36.59, 23.34, 6.09, 17.00))
# Add label position
pie3 <- pie3 %>%
  arrange(desc(class)) %>%
  mutate(lab.ypos = cumsum(Prop) - 0.5*Prop)
# set colors
mycols3 <- c("#bfcdd0", "#6e7d75", "#d0776e", "#ebbdb1", "#eaaa65", "#d6e66f")

##### set pie 4: Marital_Status
pie4 <- data.frame(class = c("Divorced", "Married", "Single"), 
                   Prop = c(8.04, 50.33, 41.63))
# Add label position
pie4 <- pie4 %>%
  arrange(desc(class)) %>%
  mutate(lab.ypos = cumsum(Prop) - 0.5*Prop)
# set colors
mycols4 <- c("#bcc0ec", "#e7a5cc", "#f281a7")
